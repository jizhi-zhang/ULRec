...
from trl.data_utils import maybe_apply_chat_template, maybe_extract_prompt, maybe_unpair_preference_dataset
...
class Mask_KTOTrainer_simple_beta(Trainer):
    r"""
    ...
    def __init__(
        ...
        use_chat_template: Optional[str] = False,
        sft_weight: Optional[float] = 1,
        # pos_length_alpha: Optional[float] = 1,
        # neg_length_alpha: Optional[float] = 1,
        ignore_id_for_kto=[67689, 60, 128009],
        # pos_reward_normalizer: Optional[float] =1,
        # neg_reward_normalizer: Optional[float] =1,
        # pos_reward_tau: Optional[float] =0.1,
        # neg_reward_tau: Optional[float] =-0.1
    ):
        # self.pos_reward_normalizer = pos_reward_normalizer
        # self.neg_reward_normalizer = neg_reward_normalizer
        # self.pos_reward_tau = pos_reward_tau
        # self.neg_reward_tau = neg_reward_tau
        # self.pos_length_alpha = pos_length_alpha
        # self.neg_length_alpha = neg_length_alpha
        # print("pos_length_alpha:" + str(self.pos_length_alpha))
        # print("neg_length_alpha:" + str(self.neg_length_alpha))
        self.ignore_id_for_kto = ignore_id_for_kto
    ...
    def compute_reference_log_probs(self, padded_batch: dict) -> dict:
        ...
        completion_logps = self.get_batch_logps_with_mask(
            completion_logits,
    ...
    def get_batch_logps_with_mask(
        ...
        # loss_mask = labels != label_pad_token_id
        exclude_values = torch.tensor(self.ignore_id_for_kto +[-100]).to(labels.device)
        loss_mask = ~(torch.isin(labels, exclude_values))
    ...
    def forward(
        ...
        chosen_length = [batch["length"][i] for i in range(len(batch["label"])) if batch["label"][i] is True]
        rejected_idx = [i for i in range(completion_logps.shape[0]) if batch["label"][i] is False]
        rejected_length = [batch["length"][i] for i in range(len(batch["label"])) if batch["label"][i] is False]


        if len(chosen_idx) != 0 and self.sft_weight != 0:
            chosen_logits = completion_logits[chosen_idx]
            chosen_labels = batch["completion_labels"][chosen_idx]
            # 计算sftloss
            sft_loss = F.cross_entropy(torch.flatten(chosen_logits[:, :-1, :], end_dim=1), torch.flatten(chosen_labels[:, 1:], end_dim=1), ignore_index=-100)
        else:
            sft_loss = 0
        ...
        # if self.aux_loss_enabled:
            # return (chosen_logps, rejected_logps, chosen_logits, rejected_logits, KL_logps, sft_loss, outputs.aux_loss)
        # else:
        return (chosen_logps, rejected_logps, chosen_logits, rejected_logits, KL_logps, sft_loss, chosen_length, rejected_length)
    ...
    def kto_loss(
        ...
        if policy_chosen_logps.shape[0] != 0 or reference_chosen_logps.shape[0] != 0:
            ...
            # chosen_logratios_length = chosen_logratios - self.pos_length_alpha * torch.tensor(chosen_length).to(chosen_logratios.device)

            # pos_beta_temp = self.beta * torch.exp(- self.pos_reward_tau * (torch.tensor(chosen_length).to(chosen_logratios.device) - self.pos_reward_normalizer))

            if self.loss_type == "kto":
                # Eqn (7) of the KTO paper (https://huggingface.co/papers/2402.01306)
                chosen_losses = 1 - F.sigmoid(self.beta * (chosen_logratios - kl))
            elif self.loss_type == "log_kto":
                # Eqn (7) of the KTO paper (https://huggingface.co/papers/2402.01306)
                chosen_losses = 1 - F.logsigmoid(self.beta * (chosen_logratios - kl))
        ...
        if policy_rejected_logps.shape[0] != 0 or reference_rejected_logps.shape[0] != 0:
            ...
            # rejected_logratios_length = rejected_logratios # - self.neg_length_alpha * torch.tensor(rejected_length).to(rejected_logratios.device)

            # neg_beta_temp = self.beta * torch.exp(- self.neg_reward_tau * (torch.tensor(rejected_length).to(rejected_logratios.device) - self.neg_reward_normalizer))


            if self.loss_type == "kto":
                rejected_losses = 1 - F.sigmoid(self.beta * (kl - rejected_logratios))
            elif self.loss_type == "log_kto":
                rejected_losses = 1 - F.logsigmoid(self.beta * (kl - rejected_logratios))
            elif self.loss_type == "apo_zero_unpaired":
    ...
    def get_batch_loss_metrics(
        ...
        if self.sft_weight != 0:
            sft_loss = forward_output[5]
        else:
            sft_loss = 0
        
        chosen_length = forward_output[6]
        rejected_length = forward_output[7]
        ...
        if all_num_chosen > 0:
            ...
            metrics["sft_loss/chosen"] = sft_loss
