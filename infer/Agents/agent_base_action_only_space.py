...
class ReactAgent:
    ...
    def step(self, idxs) -> None:
        ...
        for i, id in enumerate(idxs):
            ...
            #     action
            # except NameError:
            #     # when the action is null, stop
            #     print(f"actions are null, break")
            #     break
    ...
    def prompt_agent(self, idxs) -> str:
        return format_step(self.llm(self._build_agent_prompt_no_truncate(idxs)))
    
    def _build_agent_prompt_no_truncate(self, idxs) -> str:
...
def parse_action(list_str: list[str]):
    ...
    for string in list_str:
        ...
        if match:
            ...
            # 去掉推荐物品前后空格
            arguments.append(match.group(2).strip())
